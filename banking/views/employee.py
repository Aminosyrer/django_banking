...
def customer_list(request):
    ...
    page_count = (customer_count // 10) + (1 if customer_count % 10 != 0 else 0)
    if page > page_count:
        page = page_count
    ...
    customers = Customer.objects.all().prefetch_related('loan_applications').select_related('user', 'customer_rank')[start_index:end_index]
    for customer in customers:
        customer.pending_loan_applications = customer.loan_applications.filter(status='pending').count()

    context = {'customers': customers, 'customer_form': customer_form, 'page': page, 'page_count': page_count}
...
def loan_application_details(request, customer_pk, application_pk):
    ...
    if request.method == 'PATCH':
        ...
        elif status == 'approved_employee' and request.user.groups.filter(name='Employee').exists():
            try:
                loan_application.status = request.PATCH['status']
        ...
        elif status == 'approved' and request.user.is_superuser:
             try:
                 loan_application.status = request.PATCH['status']
